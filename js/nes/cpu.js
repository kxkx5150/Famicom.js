class CPU {
  ...
  getAddr(mode) {
    switch (mode) {
      ...
      case "IZX": {
        let adr = (this.mem.Get(this.PC[0]++) + this.X[0]) & 0xff;
        return this.mem.Get(adr) | (this.mem.Get((adr + 1) & 0xff) << 8);
      }
      case "IZY": {
        let adr = this.mem.Get(this.PC[0]++);
      ...
      case "IND": {
        let adrl = this.mem.Get(this.PC[0]++);
        let adrh = this.mem.Get(this.PC[0]++);
        let radr = this.mem.Get(adrl | (adrh << 8));
        radr |= this.mem.Get(((adrl + 1) & 0xff) | (adrh << 8)) << 8;
        return radr;
      }
      case "REL": {
        let rel = this.mem.Get(this.PC[0]++);
  ...
  execInstruction(opcode, adr) {
    switch (opcode) {
      ...
      case "SBC": {
        let value = this.mem.Get(adr) ^ 0xff;
        let result = this.A[0] + value + (this.carry ? 1 : 0);
        this.carry = result > 0xff;
        this.overflow = (this.A[0] & 0x80) === (value & 0x80) && (value & 0x80) !== (result & 0x80);
        this.A[0] = result;
      ...
      case "DEC": {
        let result = (this.mem.Get(adr) - 1) & 0xff;
        this.setZandN(result);
        this.mem.Set(adr, result);
        return;
      }
      case "DEX": {
        this.X[0]--;
      ...
      case "ROL": {
        let result = (this.mem.Get(adr) << 1) | (this.carry ? 1 : 0);
        this.carry = result > 0xff;
        this.setZandN(result);
        this.mem.Set(adr, result);
        return;
      ...
      case "STA": {
        this.mem.Set(adr, this.A[0]);
        return;
      }
      case "LDX": {
        this.X[0] = this.mem.Get(adr);
        this.setZandN(this.X[0]);
        return;
      }
      case "STX": {
        this.mem.Set(adr, this.X[0]);
        return;
      }
      case "LDY": {
        this.Y[0] = this.mem.Get(adr);
        this.setZandN(this.Y[0]);
        return;
      }
      case "STY": {
        this.mem.Set(adr, this.Y[0]);
        return;
      }
      case "TAX": {
        this.X[0] = this.A[0];
      ...
      case "PLA": {
        this.A[0] = this.mem.Get(0x100 + (++this.SP[0] & 0xff));
        this.setZandN(this.A[0]);
        return;
      }
      case "PHA": {
        this.mem.Set(0x100 + this.SP[0]--, this.A[0]);
        return;
      }
      case "PLP": {
        this.setP(this.mem.Get(0x100 + (++this.SP[0] & 0xff)));
        return;
      }
      case "PHP": {
        this.mem.Set(0x100 + this.SP[0]--, this.getP(true));
        return;
      }
      case "BPL": {
      ...
      case "BRK": {
        ...
        this.mem.Set(0x100 + this.SP[0]--, pushPc >> 8);
        this.mem.Set(0x100 + this.SP[0]--, pushPc & 0xff);
        this.mem.Set(0x100 + this.SP[0]--, this.getP(true));
        this.interrupt = true;
        this.PC[0] = this.mem.Get(0xfffe) | (this.mem.Get(0xffff) << 8);
        return;
      }
      case "RTI": {
        this.setP(this.mem.Get(0x100 + (++this.SP[0] & 0xff)));
        let pullPc = this.mem.Get(0x100 + (++this.SP[0] & 0xff));
        pullPc |= this.mem.Get(0x100 + (++this.SP[0] & 0xff)) << 8;
        this.PC[0] = pullPc;
        return;
      }
      case "JSR": {
      ...
      case "IRQ": {
        ...
        this.mem.Set(0x100 + this.SP[0]--, pushPc >> 8);
        this.mem.Set(0x100 + this.SP[0]--, pushPc & 0xff);
        this.mem.Set(0x100 + this.SP[0]--, this.getP(false));
        this.interrupt = true;
        this.PC[0] = this.mem.Get(0xfffe) | (this.mem.Get(0xffff) << 8);
        return;
      }
      case "NMI": {
        ...
        this.mem.Set(0x100 + this.SP[0]--, this.getP(false));
        this.interrupt = true;
        this.PC[0] = this.mem.Get(0xfffa) | (this.mem.Get(0xfffb) << 8);
        return;
      }
      ...
      case "SAX": {
        this.mem.Set(adr, this.A[0] & this.X[0]);
        return;
      }
      case "LAX": {
        this.A[0] = this.mem.Get(adr);
      ...
      case "DCP": {
        let value = (this.mem.Get(adr) - 1) & 0xff;
        this.mem.Set(adr, value);
        value ^= 0xff;
        let result = this.A[0] + value + 1;
        this.carry = result > 0xff;
        this.setZandN(result & 0xff);
