...
class Mapper10 extends Base {
	...
	BuildBGLine() {
		...
		var nameAddr = 0x2000 | (this.nes.ppu.PPUAddress & 0x0FFF);
		var tableAddr = ((this.nes.ppu.PPUAddress & 0x7000) >> 12) | (this.nes.ppu.IO1[0x00] & 0x10) << 8;
	...
	SetChrRom(addr) {
		if ((addr & 0x1000) === 0x0000) {
			if (!this.MAPPER_Latch0) {
				...
				this.nes.ppu.SetChrRomPage1K(0, this.MAPPER_REG[0] * 4);
				this.nes.ppu.SetChrRomPage1K(1, this.MAPPER_REG[0] * 4 + 1);
				this.nes.ppu.SetChrRomPage1K(2, this.MAPPER_REG[0] * 4 + 2);
				this.nes.ppu.SetChrRomPage1K(3, this.MAPPER_REG[0] * 4 + 3);
			} else {
				this.nes.ppu.SetChrRomPage1K(0, this.MAPPER_REG[1] * 4);
				this.nes.ppu.SetChrRomPage1K(1, this.MAPPER_REG[1] * 4 + 1);
				this.nes.ppu.SetChrRomPage1K(2, this.MAPPER_REG[1] * 4 + 2);
				this.nes.ppu.SetChrRomPage1K(3, this.MAPPER_REG[1] * 4 + 3);
			}
		} else {
			if (!this.MAPPER_Latch1) {
				//if(!this.MAPPER_REG[5]) {
				this.nes.ppu.SetChrRomPage1K(4, this.MAPPER_REG[2] * 4);
				this.nes.ppu.SetChrRomPage1K(5, this.MAPPER_REG[2] * 4 + 1);
				this.nes.ppu.SetChrRomPage1K(6, this.MAPPER_REG[2] * 4 + 2);
				this.nes.ppu.SetChrRomPage1K(7, this.MAPPER_REG[2] * 4 + 3);
			} else {
				this.nes.ppu.SetChrRomPage1K(4, this.MAPPER_REG[3] * 4);
				this.nes.ppu.SetChrRomPage1K(5, this.MAPPER_REG[3] * 4 + 1);
				this.nes.ppu.SetChrRomPage1K(6, this.MAPPER_REG[3] * 4 + 2);
				this.nes.ppu.SetChrRomPage1K(7, this.MAPPER_REG[3] * 4 + 3);
			}
		}
	}
	BuildSpriteLine() {
		var tmpBgLineBuffer = this.nes.ppu.BgLineBuffer;
		var tmpIsSpriteClipping = (this.nes.ppu.IO1[0x01] & 0x04) === 0x04 ? 0 : 8;

		if ((this.nes.ppu.IO1[0x01] & 0x10) === 0x10) {
			var tmpSpLine = this.nes.ppu.SpriteLineBuffer;
			for (var p = 0; p < 256; p++)
				tmpSpLine[p] = 256;

			var tmpSpRAM = this.nes.ppu.SPRITE_RAM;
			var tmpBigSize = (this.nes.ppu.IO1[0x00] & 0x20) === 0x20 ? 16 : 8;
			var tmpSpPatternTableAddress = (this.nes.ppu.IO1[0x00] & 0x08) << 9;

			var tmpVRAM = this.nes.ppu.VRAM;
			var tmpSPBitArray = this.nes.ppu.SPBitArray;

			var lineY = this.nes.ppu.PpuY;
			var count = 0;
			...
			if (count >= 8)
				this.nes.ppu.IO1[0x02] |= 0x20;

			else
				this.nes.ppu.IO1[0x02] &= 0xDF;
		}
	}
	GetState() {
