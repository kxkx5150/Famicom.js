class Mapper5 extends Base{
	...
	Init() {
		...
		tmp = 0;
		this.nes.ppu.SetChrRomPages1K(tmp, tmp + 1, tmp + 2, tmp + 3, tmp + 4, tmp + 5, tmp + 6, tmp + 7);
	}
	HSync(y) {
		if (y < 240) {
		...
		if (address === 0x5105) {
			...
			for (i = 0; i < 4; i++) {
				switch ((data >>> (i * 2)) & 0x03) {
					case 0:
						this.nes.ppu.VRAM[8 + i] = this.nes.ppu.VRAMS[8];
						break;
					case 1:
						this.nes.ppu.VRAM[8 + i] = this.nes.ppu.VRAMS[9];
						break;
					case 2:
						this.nes.ppu.VRAM[8 + i] = this.MAPPER_EXRAM2;
						break;
					case 3:
						this.nes.ppu.VRAM[8 + i] = this.MAPPER_EXRAM3;
						break;
				}
			}
			return;
	...
	BuildBGLine() {
		this.nes.ppu.SetChrRomPages1K(this.MAPPER_CHR_REG[1][0], this.MAPPER_CHR_REG[1][1], this.MAPPER_CHR_REG[1][2], this.MAPPER_CHR_REG[1][3],
		...
		if ((this.MAPPER_REG[0x0200] & 0x80) === 0x80) {
			...
			this.nes.ppu.SetChrRomPage1K(0, chrpage);
			this.nes.ppu.SetChrRomPage1K(1, chrpage + 1);
			this.nes.ppu.SetChrRomPage1K(2, chrpage + 2);
			this.nes.ppu.SetChrRomPage1K(3, chrpage + 3);
	...
	BuildSpriteLine() {
		this.nes.ppu.SetChrRomPages1K(this.MAPPER_CHR_REG[0][0], this.MAPPER_CHR_REG[0][1], this.MAPPER_CHR_REG[0][2], this.MAPPER_CHR_REG[0][3],
